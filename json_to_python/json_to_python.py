...
from pyclass import PyClass, PyLine, PyClassList
...
class JsonToPython:
    ...
    def dict_to_class(data: dict, name: str, classes: PyClassList) -> PyClassList:
        ...
        pyclass.add_line(PyLine(f'class {pyclass.class_name}:', 0))
        data_dict_name = f'{name}_data'
        pyclass.add_line(PyLine(f'def __init__(self, {data_dict_name}: dict):', 1))
        for key, val in data.items():
            class_property_name = Naming.camel_to_snake(key)
            if type(val) is int:
                pyclass.add_line(PyLine(f"self.{class_property_name}: int = {data_dict_name}.get('{key}')", 2))
            elif type(val) is float:
                pyclass.import_decimal = True
                pyclass.add_line(PyLine(f"{class_property_name}: float = {data_dict_name}.get('{key}')", 2))
                pyclass.add_line(PyLine(
                ...
                if val:
                    ...
                    if type(val[0]) is str:
                        pyclass.add_line(PyLine(f"self.{class_property_name}: list[str] = {data_dict_name}.get('{key}')", 2))
                    if type(val[0]) is float:
                        pyclass.import_decimal = True
                        pyclass.add_line(PyLine(f"self.{class_property_name}: list[Decimal] = [Decimal(str(item)) for item in {data_dict_name}.get('{key}')]", 2))
                    if type(val[0]) is dict:
                        last_class = JsonToPython.dict_to_class(val[0], key, classes).get_last_class()
                        pyclass.add_line(PyLine(f"self.{class_property_name}: list[{last_class.class_name}] = [{last_class.class_name}(item) for item in {data_dict_name}.get('{key}')]", 2))
            elif type(val) is dict:
                JsonToPython.dict_to_class(val, key, classes)

        pyclass.add_line(PyLine('', 0))
        pyclass.add_line(PyLine('', 0))
